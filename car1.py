...
import numpy as np
import joblib
# Importing StandardScaler and RandomForestRegressor to ensure all dependencies 
# used in the artifact files are available in the script.
...
# --- FEATURE ORDER ---
# This order is inferred from your scaler/model training and MUST be correct.
FEATURE_ORDER = [
...
def load_artifacts():
    ...
    try:
        ...
        st.error(f"Error: Preprocessor '{SCALER_FILE}' not found. Please ensure it is in the same directory.")
        return None, None
    ...
    try:
        ...
        st.error(f"Error: Model '{MODEL_FILE}' not found. Please ensure it is in the same directory.")
        return None, None
    except Exception as e:
...
def main():
    ...
    if scaler is None or model is None:
        st.warning("Cannot proceed without the necessary model artifacts.")
        return

    # --- Sidebar for User Inputs ---
    st.sidebar.header("Car Specifications Input")
    st.sidebar.markdown("Use the controls below to define the car's features.")
    ...
    if st.button("Predict Efficiency"):
        ...
        # .transform() is used because the scaler is already trained
        scaled_input = scaler.transform(input_df)

        # 4. Make the prediction
        raw_prediction = model.predict(scaled_input)[0]

        # Convert the prediction to the required binary (0 or 1) output.
        # Assuming your RandomForestRegressor was trained on a continuous MPG/efficiency score.
        # A common threshold of 0.5 is used for binary classification derived from regression/scores.
        # If your model is a RandomForestClassifier, the output will already be 0 or 1.
        
        # We'll round the output to ensure a binary result.
        final_prediction = int(np.round(raw_prediction))
        ...
        with st.expander("Show Detailed Input"):
            st.dataframe(input_df.T, use_container_width=True)
            st.markdown(f"**Raw Model Prediction Score:** `{raw_prediction:.4f}` (Rounded to `{final_prediction}`)")
